...
import com.tmt.core.exception.StaleObjectStateException;
...
public class BaseDaoImpl<T, PK> implements BaseDao<T, PK> {
	 ...
	 * 可以调用sessionTemplate完成数据库操作.<br/>
	 * 如果有多个SqlSessionTemplate 则按照名称注册
	 */
	@Autowired
	...
	protected String getStatementName(String operate) {
		...
		return StringUtils.isNotBlank(namespace) ? (new StringBuilder(namespace).append(".").append(operate).toString())
				: namespace;
	}
	...
	protected SqlSessionTemplate getSqlRunner() {
		return this.primarySessionTemplate;
	}
	...
	protected <E> Page queryForPage(String sql, Object args, PageParameters pageParameters) {
		...
		if (pageNum == PageParameters.NO_PAGINATION || pageSize == PageParameters.NO_PAGINATION) {
			...
			count = (Integer) this.getSqlRunner()
					.selectOne(getStatementName(new StringBuilder(sql).append("Stat").toString()), args);
			...
			if (count == 0) {
				...
				lst = this.getSqlRunner().selectList(getStatementName(sql), args,
