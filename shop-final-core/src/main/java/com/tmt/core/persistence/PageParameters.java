...
public class PageParameters implements Serializable {
	...
	/**
	 * serializePage :默认是 true 是否需要序列化分页条 分页条
	 * 
	 * @return
	 */
	...
	public String getSimplePagination() {
		...
		if (pageIndex == first) {// 如果是首页
			...
			sb.append("<li><a href=\"javascript:").append(funcName).append("(").append(prev).append(",")
					.append(pageSize).append(");\">&#171; 上一页</a></li>");
		}
		int begin = pageIndex - (length / 2);
		if (begin < first) {
			begin = first;
		...
		if (begin > first) {
			int i = 0;
			for (i = first; i < first + slider && i < begin; i++) {
				sb.append("<li><a href=\"javascript:").append(funcName).append("(").append(i).append(",")
						.append(pageSize).append(");\">").append(i + 1 - first).append("</a></li>");
			}
			if (i < begin) {
		...
		for (int i = end + 1; i <= last; i++) {
			sb.append("<li><a href=\"javascript:").append(funcName).append("(").append(i).append(",").append(pageSize)
					.append(");\">").append(i + 1 - first).append("</a></li>");
		}
		if (pageIndex == last) {
			sb.append("<li class=\"disabled\"><a href=\"javascript:\">下一页 &#187;</a></li>");
		} else {
			sb.append("<li><a href=\"javascript:").append(funcName).append("(").append(next).append(",")
					.append(pageSize).append(");\">").append("下一页 &#187;</a></li>");
		}
		sb.insert(0, "<ul class=\"pagination\">").append("</ul>");
		return sb.toString();
	}
