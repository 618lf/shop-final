...
import com.tmt.core.http.LocalHttpClient;
import com.tmt.core.http.handler.JsonResponseHandler;
import com.tmt.core.lock.ResourceLock;
...
public class WechatOptionServiceImpl implements WechatOptionService{
		...
		if (ticket == null || ticket.isExpired()) {
			...
			if (ticket == null || ticket.isExpired()) {
				...
				if (ticket != null && ticket.getExpires_in() != 0){
					ticket.setAddTime(DateUtils.getTimeStampNow());
	...
	private Token getToken() {
        ...
        if (token == null || token.isExpired()) {
        	token = ResourceLock.getLock(WechatConstants.LOCK_FOR_ACCESS_TOKEN).doHandler(new com.tmt.core.lock.Handler() {
        		@Override
				...
				public Token doHandle() {
	        		...
	        		if (_token == null || _token.isExpired()) {
	        			...
	        			if (_token != null && _token.getExpires_in() != 0){
	        				_token.setAddTime(DateUtils.getTimeStampNow());
