...
import com.tmt.common.utils.XmlParse;
...
public class EventMsgUserAttention extends MsgHead {
	...
	public String getQrscene() {
		...
		// 正常的扫锚关注事件
		if (StringUtil3.isNotBlank(eventKey)
				&& StringUtil3.startsWith(eventKey, "qrscene_")) {
			return StringUtil3.remove(eventKey, "qrscene_");
		}

		// weixin bug 关注时居然推送 last_trade_no_ 这样的eventkey， 系统不处理这样的事件
		else if (StringUtil3.isNotBlank(eventKey)
				&& !StringUtil3.startsWith(eventKey, "last_trade_no_")) {
			return eventKey;
		}
		return null;
	}
