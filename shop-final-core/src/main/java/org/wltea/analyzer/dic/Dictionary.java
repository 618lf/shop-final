	...
	private void loadExtDict() {
		...
		if (extDictFiles != null) {
			...
			for (String extDictName : extDictFiles) {
				// 读取扩展词典文件
				System.out.println("加载扩展词典(优先尝试绝对路径)：" + extDictName);
				File file = new File(extDictName);
				if (file.exists()) {
					try {
	...
	private void loadStopWordDict() {
		...
		if (extStopWordDictFiles != null) {
			...
			for (String extStopWordDictName : extStopWordDictFiles) {
				System.out.println("加载扩展停止词典(优先尝试绝对路径)：" + extStopWordDictName);
				// 读取扩展词典文件
				File file = new File(extStopWordDictName);
				if (file.exists()) {
					try {
						is = FileUtils.openInputStream(file);
					} catch (IOException e) {
