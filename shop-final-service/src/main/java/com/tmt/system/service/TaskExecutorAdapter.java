...
/**
 * 任务执行适配器
 * 1. 立即执行是计划之外的执行,不会改变计划的下次执行时间
 * 2. 设置的执行时间和上次执行时间比较（内部保留的），如果已过则不执行，未过则执行
 *    同时判断是否应该执行。
 * @author root
 */
public class TaskExecutorAdapter implements Job{
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		if (task != null && target != null) {
		    ...
		    task.setNextExecuteTime(context.getNextFireTime()!=null?context.getNextFireTime():task.getNextExecuteTime());
