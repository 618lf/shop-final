...
public class MenuService extends BaseService<Menu, Long> implements MenuServiceFacade {
	...
	public List<Menu> findAuthorityByGroupId(Long groupId) {
		List<Menu> menus = CacheUtils.get(new StringBuilder(Constants.CACHE_GROUP_MENUS).append(groupId).toString());
		if (menus == null) {
			menus = this.queryForList("findAuthorityByGroupId", groupId);
			if (menus == null) {
				menus = Lists.newArrayList();
			}
			CacheUtils.put(new StringBuilder(Constants.CACHE_GROUP_MENUS).append(groupId).toString(), menus);
		}
		return menus;
	}
