...
public class QrcodeController extends BaseController {
	...
	public String save(Qrcode qrcode, Model model, RedirectAttributes redirectAttributes) {
		...
		if (IdGen.isInvalidId(qrcode.getId())) {
			...
			if (msg != null) {
				addMessage(redirectAttributes, StringUtils.format("%s'%s'%s", "创建二维码", qrcode.getAppId(), "失败"));
				return WebUtils.redirectTo(Globals.adminPath, "/wechat/qrcode/form");
			}
		}
		this.qrcodeService.save(qrcode);
		addMessage(redirectAttributes, StringUtils.format("%s'%s'%s", "创建二维码", qrcode.getAppId(), "成功"));
	...
	private String sendQrcode(Qrcode qrcode) {
		...
		if (ticket != null && StringUtils.isNotBlank(ticket.getTicket())) {
	...
	private void initQc(Qrcode qrcode, Criteria c) {
		if (StringUtils.isNotBlank(qrcode.getAppId())) {
