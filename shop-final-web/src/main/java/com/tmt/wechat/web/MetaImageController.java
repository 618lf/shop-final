...
public class MetaImageController extends BaseController{
	...
	public String save(MetaImage metaImage, Model model, RedirectAttributes redirectAttributes) {
		// 判断是否修改图片
		if ((IdGen.isInvalidId(metaImage.getId()) && StringUtils.isNotBlank(metaImage.getImage()))
				|| (metaImage.getIsUpdate() != null 
				&& metaImage.getIsUpdate() == 1 && StringUtils.isNotBlank(metaImage.getImage()))) {
		...
		addMessage(redirectAttributes, StringUtils.format("%s'%s'%s", "修改图片", metaImage.getKeyword(), "成功"));
	...
	private void initQc(MetaImage metaImage, Criteria c) {
		if (StringUtils.isNotBlank(metaImage.getKeyword())) {
