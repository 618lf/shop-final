...
/**
 * 用户共用信息
 * 1. 用户的权限，菜单等，同一用户多次登录，也使用同一份数据
 * 2. 用户工具类 --是与用户相关的信息才存入此缓存中，如果信息比较大则存储到 ehcache
 * 这些个信息都是存放在session中的
 * 如果把session 存放在 mmcache 中，则这些信息就是序列化到缓存中了。
 * 直接存储到 ehcache 中  CACHE_USER 是前缀: 放入redis中
 * 最后能每次访问都绑定到当前的线程,不然每次用一次都都取一次比较麻烦
 * @author lifeng 
 */
public class UserUtils {
	...
	/**
	 * 同一用户共享（多个地方登录）
	 * Principal 是从 Session 中获取的
	 * @return
	 */
	public static User getUser(){
		
		// 优先从线程栈中获取用户信息
		User user = ThreadContext.get(SystemConstant.USER_KEY);
		Principal principal = null;
		
		// 没有数据，获取数据路径：缓存 --> 数据库获取
		if (user == null && (principal = (Principal)SecurityUtils.getSubject().getPrincipal()) != null
				&& principal.getId() != null) {
			String _key = new StringBuilder(SystemConstant.CACHE_USER).append(principal.getId()).toString();
	...
	public static Boolean isRemembered(){
		return SecurityUtils.getSubject() != null && SecurityUtils.getSubject().getPrincipal() != null && SecurityUtils.getSubject().isRemembered();
	}
	...
	public static String getUserMenus() {
		User user = getUser();
		String _key = new StringBuilder(SystemConstant.CACHE_MENU_LIST).append(user.getId()).toString();
	...
	public static void removeUser(long userId) {
		CacheUtils.getSessCache().evict(new StringBuilder(SystemConstant.CACHE_USER).append(userId).toString());
	}
	public static void removeUserCache(long userId) {
		CacheUtils.getSessCache().evict(new StringBuilder(SystemConstant.CACHE_USER).append(userId).toString());
		CacheUtils.getSessCache().evict(new StringBuilder(SystemConstant.CACHE_MENU_LIST).append(userId).toString());
	...
	public static void removeAllCache() {
		CacheUtils.getSessCache().evict(new StringBuilder(SystemConstant.CACHE_MENU_LIST).append(SystemConstant.CACHE_ALL).toString());
		Realm realm = SpringContextHolder.getBean(Realm.class);
	...
	public static void setAttribute(String key, Object value) {
		String _key = new StringBuilder().append(SystemConstant.CACHE_USER).append(UserUtils.getUser().getId()).append(SystemConstant.CACHE_DIV).append(key).toString();
	...
	public static boolean isValidateLoginCode(String useruame, boolean isFail){
		...
		// 默认设置的次数
		int dCount = Ints.defaultInteger(site.getAccountLockCount(), SystemConstant.DEFAULT_FAILE_COUNT);
		...
		// 判断当前用户的错误次数
		String key = new StringBuilder(SystemConstant.LOGIN_VALIDATE_COUNT).append(useruame).toString();
		Integer count = null;
		try{
			...
			int times = Ints.defaultInteger(site.getAccountLockTime(), SystemConstant.DEFAULT_LOCK_TIME) * 60; 
