...
public class MetaImageUpdateHandler extends AbstractUpdateHandler {

	private MetaKeywordServiceFacade metaKeywordService;

	public MetaImageUpdateHandler() {
	...
	// 图片索引
	private void meta_image_lucene(List<UpdateData> metas) {
		...
		for (UpdateData data : metas) {
			...
			if (updates.size() >= 15) {
				metaKeywordService.refresh_image(updates);
				updates.clear();
			}
		}
