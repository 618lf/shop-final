...
public class EpayUtils {
	...
	public static Epayment pay(Epayment epayment) {
		...
		if (epay != null && epay.getType() == 1) {
			...
			payment.setAmount(
					(int) (BigDecimalUtil.mul(epayment.getAmount(), BigDecimalUtil.valueOf(100)).doubleValue()));
			payment.setDesc(epayment.getRemarks());
			payment.setSpbill_create_ip(OS.ip());
			...
			if (result != null) {
				throw new EpaymentErrorException(
						StringUtils.format("【%s】%s", result.get("return_msg"), result.get("err_code_des")));
			}
		}
	 ...
	 * @return
	 */
	public static AjaxResult generateWxPayJsRequestJson(Epay epay, String ip, String openId, Payable pay,
			String notifyUrl) {
		...
		if (result != null && WechatConstants.SUCCESS.equals(result.get("return_code"))) {
			return AjaxResult.success(SignUtils.generateMchPayJsRequestJson(pp.getNonce_str(), result.get("prepay_id"),
					config.getMchId(), config.getMchKey()));
		}
	...
	public static Payment wxCheckPayment(Epay epay, Payable pay) {
		...
		if (result != null && WechatConstants.SUCCESS.equals(result.get("return_code"))
			...
			payment.setAmount(
					BigDecimalUtil.div(BigDecimalUtil.valueOf(result.get("total_fee")), BigDecimal.valueOf(100)));
			payment.setPayer(result.get("openid"));
			payment.setPaymentDate(DateUtils.getFormatDate(result.get("time_end"), "yyyyMMddHHmmss"));
			return payment;
		}
	...
	/**
	 * 发起退款申请，申请成功之后 不支持多次退款（一个订单一次性退款）
	 * 
	 * @param payment
	 */
