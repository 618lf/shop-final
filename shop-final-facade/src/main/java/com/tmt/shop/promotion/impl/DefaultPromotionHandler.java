...
public abstract class DefaultPromotionHandler implements PromotionHandler {
	...
	public Boolean doHandler(PromotionWrap promotion) {
		...
		try {
			flag = this.doInnerHandler(promotion, promotion.getPromotion(), promotion.getOrder(), promotion.getItem(),
					promotion.getAmount(), promotion.getQuantity());
		} catch (Exception e) {
	...
	protected void doPostHandler(PromotionWrap wrap, PromotionResult flag) {
		...
		// 有效 且设置为不能使用优惠券
		Promotion promotion = wrap.getPromotion();
		Order order = wrap.getOrder();
		if (flag.getReduce() != null && promotion.getIsCouponAllowed() != null && promotion.getIsCouponAllowed() == 0) {
	 ...
	 * @return
	 */
	protected abstract PromotionResult doInnerHandler(PromotionWrap wrap, Promotion promotion, Order order,
			OrderItem item, BigDecimal amount, int quantity);
}
