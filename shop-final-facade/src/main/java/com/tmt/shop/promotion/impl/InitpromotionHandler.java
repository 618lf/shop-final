...
public class InitpromotionHandler extends DefaultPromotionHandler {
	...
	protected PromotionResult doInnerHandler(PromotionWrap wrap, Promotion promotion, Order order, OrderItem item,
		...
		// 本身的有效性 -- 后面的不需要处理
		if (promotion.getIsEnabled() == 0
				|| !(promotion.getEndDate() != null && DateUtils.after(promotion.getEndDate(), now))
				|| !(promotion.getBeginDate() != null && DateUtils.before(promotion.getBeginDate(), now))) {
			return PromotionResult.success("促销已过期");
		}

		// 校验是否符合规格
		if (!(promotion.getPs() == null || promotion.getPs().isEmpty()
				|| promotion.getPs().containsKey(item.getProductId()))) {
		...
		int _quantity = 0;
		if (promotion.getType() == 2 || promotion.getType() == 3 || promotion.getType() == 5
				|| promotion.getType() == 8) {
			...
			for (OrderItem _item : items) {
				// 满足这些条件才需要汇总
				if (!IdGen.isInvalidId(_item.getPromotions()) && promotion.getId().compareTo(_item.getPromotions()) == 0
						&& (promotion.getPs() == null || promotion.getPs().isEmpty()
								|| promotion.getPs().containsKey(_item.getProductId()))) {
