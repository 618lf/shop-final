 ...
 * @author lifeng
 */
public class RefundProcessEventHandler extends DefaultEventHandler{

	@Override
	protected Boolean doInnerHandler(OrderEvent event) {
		
         if (event.getOpt() == OrderOpts.REFUND_PROCESS) {
			
			 Order order = orderService.get(event.getOrderId());
			
			 try {
				// 2.收货
				NoticeSetting setting = noticeSettingService.get((byte)8);
