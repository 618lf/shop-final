...
(function($) {
	...
	/**
     * 获取元素的outerHTML --- 包括html() 都会有一个问题
     * 如果动态改变的值无法copy。需要用到 clone()；
     */
...
Public.defaultGrid = function(params){
	...
	var defaults = {
		    ...
		    beforeRequest:function(){
		    	...
		    	//var _p = formObj.data('init-page'), _r = formObj.data('init-rows');
		    	//if(!!_p && !!_r) {that.jqGrid('setGridParam', {page:_p, rows: _r});}
...
Public.doMemoryQuery = function(gridName) {
	...
	var _cookie = (function(form) {
		...
		//添加分页支持
		//obj[{name: 'page', value: parseInt($('#' + (gridName||'grid')).jqGrid('getGridParam', 'page'))}];
		//obj[{name: 'rows', value: parseInt($('#' + (gridName||'grid')).jqGrid('getGridParam', 'rows'))}];
		return obj;
	})(formObj);
	if( !!_cookie ) {
