...
var THISPAGE = {
	...
	loadGrid : function(){
		...
		$('#grid').jqGrid(
			Public.defaultGrid({
				url: '${ctx}/gen/table/page?timeid='+ Math.random(),
				height:init.h,
				shrinkToFit:!1, 
				rownumbers: !0,//序号
				...
				colModel: [
	                ...
	                {name:'name', index:'name', width:250, align:'left', sortable:false},
	...
	addEvent : function(){
		...
		var delBusinessType = function(checkeds){
			if( !!checkeds  ) {
				...
				Public.deletex("确定删除选中的业务类型？","${ctx}/gen/table/delete",param,function(data){
					if(!!data.success) {
		...
		$('#dataGrid').on('click','.edit',function(e){
			window.location.href = "${ctx}/gen/table/form?id="+$(this).attr('data-id');
		});
		$('#dataGrid').on('click','.delete',function(e){
			delBusinessType($(this).attr('data-id'));
		});
		$(document).on('click','#addBtn',function(e){
			window.location.href = "${ctx}/gen/table/form";
		});
