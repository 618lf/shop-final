...
var THISPAGE = {
	...
	loadGrid : function(){
		...
		$('#grid').jqGrid(
			Public.defaultGrid({
				url: '${ctx}/wechat/meta/text/page?timeid='+ Math.random(),
				height:init.h,
				shrinkToFit:!1, 
                rownumbers: !0,//序号
				multiselect:true,//定义是否可以多选
				multiboxonly: false,
				colNames: ['ID', '关键词', '内容', '创建时间', '修改时间', ''],
				colModel: [
	...
	addEvent : function(){
		Public.initBtnMenu();
		var delMetaText = function(checkeds){
			if( !!checkeds && checkeds.length ) {
				...
				Public.deletex("确定删除选中的文本回复？","${ctx}/wechat/meta/text/delete", param, function(data){
					if(!!data.success) {
						Public.success('删除文本回复成功');
						Public.doQuery();
					} else {
				...
				Public.error("请选择要删除的文本回复!");
			}
		};
		$('#dataGrid').on('click','.edit',function(e){
			var url = "${ctx}/wechat/meta/text/form?id="+$(this).data('id');
		    Public.openOnTab('metaText-edit', '编辑文本回复', url);
		});
		$('#dataGrid').on('click','.delete',function(e){
			delMetaText($(this).data('id'));
		});
		$(document).on('click','#addBtn',function(e){
			var url = "${ctx}/wechat/meta/text/form";
			Public.openOnTab('metaText-add', '添加文本回复', url);
		});
		$(document).on('click','#delBtn',function(e){
			var checkeds = $('#grid').getGridParam('selarrrow');
			delMetaText(checkeds);
		});
	}};
$(function(){
	THISPAGE._init();
